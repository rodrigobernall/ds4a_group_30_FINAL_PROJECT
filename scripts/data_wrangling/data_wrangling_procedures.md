...
1. We unzipped each and every file and deleted a column that was a by-product of the Stata to CSV conversion. In order to do that, we ungzipped every file in each folder, used `cut` (from Bash) to get rid of the excess column and gzipped again.
...
1. We gzipped the files again.
1. We used `psql \copy` to upload the gzipped files to the AWS RDS PostgreSQL instance.
